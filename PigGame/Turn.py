    ...
    def play_turn(self):
        ...
        while keep_rolling:
            ...
            if self.is_one(roll_value):
                break  # checks if the roll result is a 1, sets turn_total to 0 and breaks out of the loop
            self.player.turn_total += roll_value
            print(
                f"{self.player.username}'s current turn total is {self.player.turn_total}"
            )

            if self.player.game_score + self.player.turn_total >= self.winning_score:
                break  # checks if it has reached a winning score and breaks out of the loop

            keep_rolling = (
                self.player.is_rolling()
            )  # asks the player roll/hold/quit and returns r/h/q

            if (
                keep_rolling == "h"
            ):  # breaks out of the loop and ends the turn allowing the return of the turn total
                break
            elif (
                keep_rolling == "q"
            ):  # returns -1 instead of the turn total and is then used in Game to break out of the loop/Game
                return -1
